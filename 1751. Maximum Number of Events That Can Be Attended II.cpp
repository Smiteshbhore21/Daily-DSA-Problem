...
public:
    ...
    int attendEvent(vector<vector<int>>& events, int pos, int k) {
        ...
        int attent_event = attendEvent(events, next, k - 1) + events[pos][2];
        return mem[pos][k] = max(skip_event, attent_event);
    }
    int maxValue(vector<vector<int>>& events, int k) {
        ...
        for (int i = 0; i < n; i++) {
            next_event[i] = upper_bound(events.begin() + i, events.end(),

...
public:
    ...
    int solve(int i, int j, int d, bool canTurn, int val,
        ...
        if (canTurn) {
            int turnAndMove =
                max(keepMoving, 1 + solve(i_, j_, (d + 1) % 4, false,
                                          val == 2 ? 0 : 2, grid));
...
//         }
//         int result = 0;
//         int keepMoving = 1 + solve(i_, j_, d, canTurn, val == 2 ? 0 : 2,
//         grid); result = max(result, keepMoving); if (canTurn) {
//             int turnAndMove = max(keepMoving, 1 + solve(i_, j_, (d+1) % 4,
//             false,
//                                                    val == 2 ? 0 : 2, grid));
//             result = max(result, turnAndMove);
//         }
...
//                 if (grid[i][j] == 1) {
//                     for (int d = 0; d <= 3; d++) {
//                         result = max(result, 1 + solve(i, j, d, true, 2,
//                         grid));
//                     }
//                 }
//             }
